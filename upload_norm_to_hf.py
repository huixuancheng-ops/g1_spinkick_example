"""Upload g1_spinkick_sweep_norm iter-4499 checkpoints to HuggingFace.

Companion to upload_to_hf.py for the obs-normalization sweep (train_norm.sh).
Differences from upload_to_hf.py:

  * Only the iter-4499 checkpoint per seed.
  * Uploads to a separate subfolder (default: analysis/spinkick).
...
SOURCE = Path("logs/rsl_rl/g1_spinkick_sweep_norm")
REQUIRED_ITERS = (4499,)
...
def commit_with_retry(api, repo_id, batch, idx, message):
  ...
  while True:
    ...
    try:
      ...
      if code == 429:
        print(f"  [batch {idx}] 429 rate-limited (attempt {attempt}), sleeping 15min...")
        time.sleep(15 * 60)
      elif code is not None and 500 <= code < 600:
        print(f"  [batch {idx}] HTTP {code} (attempt {attempt}), sleeping {backoff}s...")
        time.sleep(backoff)
        backoff = min(backoff * 2, 600)
      else:
        raise
    except (ConnectionError, TimeoutError, OSError) as e:
      print(f"  [batch {idx}] network error ({e}) (attempt {attempt}), sleeping {backoff}s...")
      time.sleep(backoff)
      backoff = min(backoff * 2, 600)
...
def main():
  ...
  parser.add_argument(
    "--source",
    default=str(SOURCE),
    help="Local sweep directory to scan (default: logs/rsl_rl/g1_spinkick_sweep_norm)",
  )
  parser.add_argument(
    "--subfolder",
    default="analysis/spinkick",
  ...
  for i in range(0, total, args.batch_size):
    ...
    commit_with_retry(
      ...
      batch,
      idx,
      f"Upload spinkick norm ckpts batch {idx}/{n_batches}",
    )
    done += len(batch)
